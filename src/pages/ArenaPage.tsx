...
import { FileText, HeartPulse, Network, Users } from "lucide-react";
...
function readPanelWidth(): number {
  try {
    ...
    if (stored >= PANEL_MIN_WIDTH && stored <= PANEL_MAX_WIDTH) return stored;
  } catch { /* ignore */ }
...
export default function ArenaPage() {
  ...
  const handleResize = useCallback((delta: number) => {
    setPanelWidth((w) => {
      const next = Math.max(PANEL_MIN_WIDTH, Math.min(PANEL_MAX_WIDTH, w + delta));
      try {
  ...
  const tabs = useMemo<RightPanelTab[]>(() => {
    ...
    return list;
  }, [t, hasDocument, argumentMapEnabled, thesesCount, relationshipsCount]);
    ...
    <>
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden">
          ...
          {isDesktop && <ResizeDivider onResize={handleResize} />}
          <RightPanel tabs={tabs} width={panelWidth} />
        </div>
      </div>
    </>
  );
